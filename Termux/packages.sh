...
then
grep -qF -- 'lampac' '.bashrc' || echo 'tmux split-window -v -t pac:0 "proot-distro login debian -- bash /home/lampac_updater.sh"'  >> .bashrc
fi
;;
...
then
grep -qF -- 'jackett' '.bashrc' || echo 'tmux split-window -v -t pac:0 "proot-distro login debian -- /home/Jackett/./jackett"'  >> .bashrc
fi
;;
3)
...
then
grep -qF -- 'torrserver' '.bashrc' || echo 'tmux split-window -v -t pac:0 "proot-distro login debian -- /home/torrserver/torrserver -p 8091 -a -d /home/config/torrserver"'  >> .bashrc
fi
;;
4)
...
if [ -d "$PREFIX/etc/tor" ];
then
grep -qF -- '-t pac:0 tor' '.bashrc' || echo 'tmux split-window -v -t pac:0 tor'  >> .bashrc
fi
;;
7)
